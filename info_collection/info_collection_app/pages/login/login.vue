    ...
    <view class="page_login">
        ...
        <view class="login_form">
            <view class="input">
                ...
                <input type="text" v-model="username" placeholder="请输入用户账号" @blur="showimg">
                <view class="img">
    ...
    export default {
        ...
        methods: {
            ...
            login() {
                if (this.code!='') {
                    ...
                    this.$api.request({
                        ...
                        if (data.message.startsWith('密码强度不足')) {
                            uni.showToast({
                                icon: 'none',
                                title: '密码必须包含字母（大写或小写）、数字和特殊符号的组合,至少8个字符，最多20个字符，请按要求修改！'
                            });
                            setTimeout(function() {
                                uni.navigateTo({
            ...
            showimg(){
                ...
                this.$api.request({
                    url: '/login/generate_img',
                    method: 'POST',
                    data: data
                }).then((data)=> {
                    this.img='data:;base64,' + data.img_stream
                })
            },
        }
    }
